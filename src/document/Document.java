...
public abstract class Document {
	...
	protected int countSyllables(String word) {
		...
		for(int i = 0; i < wordArr.length; i++){
			if (vowelList.contains(wordArr[i])) {
				currIsVowel = true;
			}	
			if (currIsVowel && !vowelList.contains(wordArr[i])) {
				totSyl++;
				currIsVowel = false;
			}
		}		
		if (totSyl > 0 && vowelList.contains(wordArr[wordArr.length-1])) {
			totSyl++;
		}
		
		if (wordArr.length == 1 && vowelList.contains(wordArr[0])) {
			totSyl++;
		}
		
		// saftey valve for effing up above (every word should have a syllable)
		if (totSyl == 0) {
			totSyl++;
		}
	...
	public double getFleschScore() {
		...
		double numSent = getNumSentences();
		double numSyl = getNumSyllables();
		
		double wordPerSent = numWords/numSent;
		double sylPerWord = numSyl/numWords;
		
	    return (206.835 - 1.015 * wordPerSent - 84.6 * sylPerWord);
	}
